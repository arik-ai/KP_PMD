...
if (!isset($_GET['id'])) {
    header("Location: surat_keputusan.php?message=invalid_id");
    exit;
}
...
$queryCheck = "SELECT dokumen_keputusan FROM surat_keputusan WHERE id_keputusan = ?";
        ...
        if (isset($_FILES['file_surat']) && $_FILES['file_surat']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array(strtolower($fileExtension), $allowedExtensions) || !in_array($fileType, $allowedMimeTypes)) {
                ...
                if (move_uploaded_file($fileTmpPath, $destPath)) {
                    ...
                    $query = "UPDATE surat_keputusan SET dokumen_keputusan = ? WHERE id_keputusan = ?";
                    $stmt = $conn->prepare($query);
                    $stmt->bind_param("si", $fileName, $idSuratKontrak);

                    if ($stmt->execute()) {
                        header("Location: surat_keputusan.php?message=upload_success");
                        exit;
                    } else {
        ...
        <div class="container">
            ...
            <?php if (isset($errorMsg)): ?>
                <div class="error-message">
                    ...
                    <a href="surat_keputusan.php" class="btn btn-secondary">Kembali</a>
                </div>
            <?php else: ?>
                <form action="upload_keputusan.php?id=<?= htmlspecialchars($idSuratKontrak); ?>" method="POST" enctype="multipart/form-data">
