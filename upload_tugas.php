...
if (!isset($_GET['id'])) {
    header("Location: surat_tugas.php?message=invalid_id");
    exit;
}
...
$queryCheck = "SELECT dokumen_tugas FROM surat_tugas WHERE id_tugas = ?";
        ...
        if (isset($_FILES['file_surat']) && $_FILES['file_surat']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array(strtolower($fileExtension), $allowedExtensions) || !in_array($fileType, $allowedMimeTypes)) {
                ...
                if (move_uploaded_file($fileTmpPath, $destPath)) {
                    ...
                    $query = "UPDATE surat_tugas SET dokumen_tugas = ? WHERE id_tugas = ?";
                    $stmt = $conn->prepare($query);
                    $stmt->bind_param("si", $fileName, $idSuratKontrak);

                    if ($stmt->execute()) {
                        header("Location: surat_tugas.php?message=upload_success");
                        exit;
                    } else {
...
<head>
    ...
    <title>Unggah File Surat Tugas</title>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="container">
            <h2>Upload File Surat Tugas (.PDF)</h2>
            <?php if (isset($errorMsg)): ?>
                <div class="error-message">
                    <?= htmlspecialchars($errorMsg); ?>
                    <a href="surat_tugas.php" class="btn btn-secondary">Kembali</a>
                </div>
            <?php else: ?>
                <form action="upload_tugas.php?id=<?= htmlspecialchars($idSuratKontrak); ?>" method="POST" enctype="multipart/form-data">
